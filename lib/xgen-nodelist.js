...
/**
 * Add elements to the NodeList.
 * @param a nodelist, an array of nodes or a single node
 */
XGenNodeList.prototype.addElements = function(nodeList) {
	if (typeof nodeList === 'undefined') return;

	// Duck typing to support, nodelist arrays and single elements
	// It quacks like a DOM nodeList
	if (typeof nodeList.item === 'function' && typeof nodeList.length === 'number') {
		for (var i = 0 ; i < nodeList.length ; i++) {
			Array.prototype.push.call(this, nodeList.item(i));
		}
	}
	// It quacks like an array, xPath array or jQuery (TODO fix this so XPath returns proper NodeLists)
	else if (typeof nodeList.length === 'number') {
		for (var i = 0 ; i < nodeList.length ; i++) {
			Array.prototype.push.call(this, nodeList[i]);
		}
	}
	// It quacks like a node, all we ever do to nodes is appendChild()
	else if (typeof nodeList.appendChild === 'function') {
		Array.prototype.push.call(this, nodeList);
	}
	else {
		throw new Error("Context type not supported: " + typeof nodeList);
	}
};
 ...
 * @public
 */
XGenNodeList.prototype.each = function(callback) {
	var tailNodes = new XGenNodeList(this.xGen);
	for (var i = 0 ; i < this.length ; i++) {
		tailNodes.add(callback(i, this[i], this.xGen));
	}
	return tailNodes;
};
